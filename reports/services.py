...
class ReportTooLargeError(Exception):
    """Raised when a period is too long to build synchronously."""


# Same 20 minute inactivity gap used on the device detail page
COOKING_GAP_SECONDS = 20 * 60

# Detail tables are capped so a long period cannot blow up the PDF / request.
DETAIL_ROW_LIMIT = 200000000

# Periods longer than this are generated by the background worker and emailed.
SYNC_MAX_DAYS = 14

# Readings are streamed from the database in chunks of this size.
READING_CHUNK_SIZE = 2000
...
def build_device_usage(user, start, end, organization=None):
    ...
    # Stream the readings and accumulate per device so a long period never
    # holds every reading in memory. Math is identical to device_detail:
    # per-reading tariff cost and the 20 minute inactivity gap rule.
    acc = {}
    for r in readings.iterator(chunk_size=READING_CHUNK_SIZE):
        row = acc.get(r.deviceid)
        if row is None:
            device = device_map.get(r.deviceid)
            row = {
                "deviceid": r.deviceid,
                "device": device,
                ...
                "cooking_minutes": 0.0,
                "cost": 0.0,
                "first_seen": timezone.localtime(r.time),
                "last_seen": timezone.localtime(r.time),
                "_prev_time": None,
                "_event_start": None,
                "_event_end": None,
            }
        ...
        row["kwh"] += kwh
        row["cost"] += kwh * rate
        row["last_seen"] = timezone.localtime(r.time)

        prev_time = row["_prev_time"]
        if prev_time is not None and (r.time - prev_time).total_seconds() > COOKING_GAP_SECONDS:
            # close the running event
            row["events"] += 1
            row["cooking_minutes"] += (
                row["_event_end"] - row["_event_start"]
            ).total_seconds() / 60
            row["_event_start"] = None

        if row["_event_start"] is None:
            row["_event_start"] = timezone.localtime(r.time)
        row["_event_end"] = timezone.localtime(r.time)
        row["_prev_time"] = r.time

    rows = []
    for row in acc.values():
        if row["_event_start"] is not None:
            row["events"] += 1
            row["cooking_minutes"] += (
                row["_event_end"] - row["_event_start"]
            ).total_seconds() / 60

        row.pop("_prev_time", None)
        row.pop("_event_start", None)
        row.pop("_event_end", None)
        row["cooking_hours"] = row["cooking_minutes"] / 60
        rows.append(row)
...
def build_report_context(user, start, end, sections=None, organization=None,
                         allow_long=False):
    if not allow_long and period_needs_background(start, end):
        raise ReportTooLargeError(
            f"Periods longer than {SYNC_MAX_DAYS} days must be generated in the background."
        )
    ...
    # ---- COUNTS (aggregated in the database) + CAPPED DETAIL ROWS ----
    ...
    inventory_units = (
        inventory.aggregate(total=Sum("quantity"))["total"] or 0
        if inventory_count
        else 0
    )

    customers = list(customers[:DETAIL_ROW_LIMIT])
    sales = list(sales[:DETAIL_ROW_LIMIT])
    inventory = list(inventory[:DETAIL_ROW_LIMIT])
    devices = list(devices[:DETAIL_ROW_LIMIT])
    transactions = list(transactions[:DETAIL_ROW_LIMIT])

    device_usage_count = len(device_usage)
    device_usage = device_usage[:DETAIL_ROW_LIMIT]
    ...
    return {
        ...
        "truncated": {
            "customers": _truncation(customers, customers_count),
            "sales": _truncation(sales, sales_count),
            "inventory": _truncation(inventory, inventory_count),
            "devices": _truncation(devices, devices_count),
            "device_usage": _truncation(device_usage, device_usage_count),
            "transactions": _truncation(transactions, transactions_count),
        },
        "summary": {
...
def _truncation(rows, total):
    """Return {'shown', 'total'} when the table was capped, otherwise None."""
    if total > len(rows):
        return {"shown": len(rows), "total": total}
    return None


def period_needs_background(start, end):
    """Long periods are rendered by the worker and emailed instead of downloaded."""
    return (end.date() - start.date()).days + 1 > SYNC_MAX_DAYS
